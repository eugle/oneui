...
var getColor = exports.getColor = function getColor(c, o, over) {
    ...
    //如果是默认单或双数值颜色，判断并组合;
    if (c && c.length <= 2) {
        var current = Object.keys(_color.DefaultColor).includes(c) ? _color.DefaultColor[c] : c;
        currentColor = o || o === 0 ? 'rgba(' + _color.DefaultColor[current + 'RGB'] + ',' + opacity + ')' : over ? _color.DefaultColor[current + 'G'] : _color.DefaultColor[current];
    }
    console.log('颜色参数: ', c);
    //如果同时给了颜色和透明度 [bg,bgo]
